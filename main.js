...
function startEngine() {
  ...
  document.getElementById('lightbox-backdrop').addEventListener('click', Lightbox.close);
  window.addEventListener('keydown', (e) => { if (e.key === 'Escape') Lightbox.close(); });
...
function goToScene(key) {
  Popup.close();
  const prevDef = currentSceneKey ? window.Rooms.getScene(currentSceneKey) : null;
...
function loop(now) {
  ...
  if (!popupOpen) {
    let dx = 0, dy = 0;    if (keys['ArrowLeft'] || keys['a']) dx -= 1;
    if (keys['ArrowRight'] || keys['d']) dx += 1;
    if (keys['ArrowUp'] || keys['w']) dy -= 1;
    if (keys['ArrowDown'] || keys['s']) dy += 1;
    const len = Math.hypot(dx, dy) || 1;
    playerPos.x = clamp(playerPos.x + (dx / len) * PLAYER_SPEED * dt, 0, viewW - PLAYER_SIZE);
    playerPos.y = clamp(playerPos.y + (dy / len) * PLAYER_SPEED * dt, 60, viewH - PLAYER_SIZE - 20);
...
function updateProximity() {
  const px = playerPos.x + PLAYER_SIZE / 2, py = playerPos.y + PLAYER_SIZE / 2;
  let found = null;
